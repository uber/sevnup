 ...
 * TODO (joseph) This should be moved to vnodeStore
...
VNodeStore.prototype.loadAllKeys = function loadAllKeys(done) {
    ...
    // TODO (joseph): Make more efficient/less loops
    // TODO (joseph): myVNodes is a hash now
    var nodesForRelease = utils.inHashAnotB(myVNodes, newVNodes);
    var nodesToRecover = utils.inHashAnotB(newVNodes, myVNodes);

    // do diff on vnodes, if it's released, send release for each key
    async.parallel([
