 ...
 */
VNodeStore.prototype.loadVNodeKeys = function loadVNodeKeys(vnodeName, forEachKey, done) {
    var self = this;
    var vnodeKeyArray = [];
    vnodeKeyArray = self.loadVNodeKeysFromStorage(vnodeName, function(err, keys) {
		self.onKeysLoaded(err, vnodeName, keys, forEachKey, function(err){
            done(err);
        });
	});
};
 ...
 * It then uses forEachKey function for the specific business logic that the service
 * wants to call on each key.  If the forEachKey returns false, we will remove the
 * key from bookkeeping.
 ...
 * @param {function} forEachKey The method to be called onthe recovery of each key.
 * @param {function} done The method to call when all keys have been handled.
 */
VNodeStore.prototype.onKeysLoaded = function onKeysLoaded(err, vnodeName, keys, forEachKey, done) {
    var self = this;
    var nodeKeyArray = keys || [];
    var keysToRemove = [];
    if (typeof forEachKey === 'function') {
        async.filter(vnodeKeyArray, forEachKey, function(completedKeys) {
            ...
            async.eachLimit(
                ...
                function (item, callback) {
                    return self.removeKeyFromVNode(vnodeName, item, callback);
                },
                function onKeysLoadedComplete(err) {
                    //TODO (joseph): when logging exists, log here.
                    done(err);
                }
            );
        });
